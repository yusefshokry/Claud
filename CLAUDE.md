...
**Which session runs routines (updated 2026-09-26):** The cloud session (`session_01VkvV3dT2RKaDYhpKx5JxgF`) is the **primary** runner for all routines, with no gate. Yusef's local "Personal Assistant" session (`session_01658mKS1vFHC6JM1ewijJRu`, Remote Control on his own computer) goes `computer_unreachable` whenever his computer sleeps, so it can't be relied on. The cloud session can't wake it. For a few hours earlier that day the cloud copies were gated as backup-only; Yusef reversed that because the local session kept disconnecting. If the local session has its own copy of a routine (e.g. its 8:30am finance brief), duplicates are possible while it's online.
...
- **"Plan Gap Filler"** (`trig_01RQRvBrpVCg7Dnn75aEMKdh`, daily 12:45 UTC / 8:45am, 30 min before the planner) — finds open Goals/Projects/Tasks with no clear next action and adds 2–5 numbered, verb-first sub-tasks under at most 3 of them per run ("Ask <person> for <thing>" when info is missing, never a guess). Respects the Reston/GM holds and the Linework lock, never edits or checks Yusef's own items, silent when there are no gaps.
...
- **"Weekly Systems Retro"** (`trig_01SJ5X98KrJZ9i4uDrcB1sAd`, Sundays 22:30 UTC / 6:30pm) — plan-vs-actual readout logged to the Time Block Log page (https://app.notion.com/p/3dee4241b1a381b1af1dce5a74154ebd). It proposes planner tweaks but only applies them if he says yes.
- The "Morning brief", "Morning time-block snapshot" and "Evening plan vs. actual" routines mentioned in earlier versions of this file are not in the cloud session's trigger list. The snapshot/evening pair never actually existed.

- **"Daily time-block planner"** (trigger `trig_018QcSWFB6jqndWKZDiv8Fgn`, recreated 2026-09-26 replacing `trig_01SxVC4k5qacHY1gwWhAWht8`, fires 13:15 UTC / ~9:15 AM ET) — reads all the calendars above plus open Notion INBOX tasks (`Area (1)` contains "Task", not checked), then auto-schedules new blocks into real open gaps. Key standing rules baked into this Routine (see the trigger's own prompt for full detail — this is a summary for other sessions/environments):
...
- **Standing permanent full permission (given 2026-09-22, widened 2026-09-22, generalized to everything 2026-09-22): default to acting, not asking.** He'd rather have something done and fix a mistake afterward than be stopped for approval mid-task. This applies across the board — calendar (creating/moving/deleting events, inviting people), Notion (pages, tasks, databases, routine prompts), Gmail, and any other tool available in this environment — not just the time-block planning system. Look up whatever's needed on your own (attendee emails from calendar/Gmail history, task URLs, etc.), do the thing, then report what was done. Don't pause mid-task to ask "should I proceed" or preview a plan before acting. The only things that still warrant a check-in first are truly catastrophic/irreversible actions with no fix-it-afterward path (e.g. sending real money, git force-push or history rewrite, permanently deleting an entire database/workspace with no recovery) — genuinely rare, and even then err toward finding the reversible version of the action rather than stalling.
  - **A clarifying question is still asking (added 2026-09-23, after this recurred yet again).** Using a question tool, presenting two-or-more options for him to pick between ("you can do X, or I can do Y"), or pausing on a factual ambiguity all count as asking permission and are covered by the same ban. When something is genuinely unclear (which of several tasks a comment referred to, which default to use), pick the single most reasonable read, act on it immediately, and state the assumption afterward so he can correct it if wrong — never lay out paths, never pause first. This applies in live conversation, not just inside the automations.
